...
static void* xipp_read_fn(void* arg)
{
	...
	unsigned int* elecs_in = (unsigned int*) malloc(sizeof(unsigned int) * xippdev->NUM_CH);
	for (unsigned int i = 0; i < xippdev->NUM_CH; i++) {
		elecs_in[i] = i;
	}
...
int xipp_open_device(struct devmodule* dev, const char* optv[])
{
	...
	// Get channels
	xippdev->NUM_CH = atoi(optv[NUM_CH]);

	int max_elecs = xippdev->NUM_CH;
	unsigned int elecs_out[max_elecs];
	memset(elecs_out, 0, sizeof(elecs_out));

	unsigned int* elecs = elecs_out;

	int n = xl_list_elec(elecs, max_elecs, "");
	if (n < 0) {
	...
	for (int i = 0; i < xippdev->NUM_CH; i++) {
		if (xl_signal_set(elecs[i], stream, 1) < 0) {
